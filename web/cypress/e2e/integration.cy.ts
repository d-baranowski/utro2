    ...
    cy.get('body').then(($body) => {
      ...
      if (hasNoOrgDialog && !hasOrgSwitcher) {
            ...
            if ($createBody.find('[data-testid="create-organisation-submit"]').length > 0) {
              ...
              cy.get('body', { timeout: 15000 }).then(($afterCreate) => {
                if (!$afterCreate.find('[data-testid="no-organisation-dialog"]').length) {
                  ...
                  cy.getByTestId('organisation-switcher-button', { timeout: 10000 }).should(
                    'be.visible'
                  );
                } else {
        ...
        cy.getByTestId('organisation-switcher-button').then(($button) => {
          ...
          cy.get('[data-testid^="organisation-option-"]', { timeout: 5000 }).should(
        ...
        cy.get('body')
          .should('contain.text', 'Test Organisation')
  ...
  it('should handle authentication persistence across page reloads', () => {
    ...
    cy.get('body').then(($beforeReload) => {
      const hasOrgSwitcher =
        $beforeReload.find('[data-testid="organisation-switcher-button"]').length > 0;
      const hasNoOrgDialog =
        $beforeReload.find('[data-testid="no-organisation-dialog"]').length > 0;
